...
class CycleMaskPointHead(nn.Module):
    ...
    def __init__(self,
        ...
        off_channels: int=in_channels//2+1
        ...
        self.cyclefc = CycleFC(off_channels, fc_channels, (1, 1), 1, 0)
        # for _ in range(num_fcs):
        #     cyclefc = CycleFC
        #     self.fcs.append(cyclefc)
    ...
    def init_weights(self):
        ...
        normal_init(self.cyclefc, std=0.001)
    ...
    def forward(self, fine_grained_feats, coarse_feats):
        ...
        """
        coarse_feats=torch.cat([coarse_feats, coarse_feats], dim=1)
        ...
        if self.coarse_pred_each_layer:
            coarse_feats = torch.unsqueeze(coarse_feats, dim=3)
            x = torch.cat((x, coarse_feats), dim=1)
            coarse_feats = torch.squeeze(coarse_feats, dim=3)
        x = torch.squeeze(x, dim=3)
        # print(self.fc_logits(x).shape)
...
class CycleFC(nn.Module):
    ...
    def gen_offset(self):
        ...
        offset (Tensor[batch_size, 2 * offset_groups * kernel_height * kernel_width,
            ...
            convolution kernel.
        """
        offset = torch.empty(1, self.in_channels, 1, 1)
    ...
    def forward(self, input: Tensor) -> Tensor:
        ...
        return deform_conv2d_tv(input, self.offset.expand( B,W, H, -1), self.weight, self.bias, stride=self.stride,padding=self.padding, dilation=self.dilation)
