...
class SparseMaskPointHead(nn.Module):
    ...
    def __init__(self,
        ...
        self.sparse_mlp = sMLPBlock(65,65,65)
    ...
    def init_weights(self):
        ...
        normal_init(self.sparse_mlp, std=0.001)
...
#         self.fuse=nn.Conv1d(3*c, c, kernel_size=1, stride=1, padding=0)
    
#     def forward(self,x):
#         x_h=self.proj_h(x.permute(0,2,1)).permute(0,2,1)
...
class sMLPBlock(nn.Module):
    def __init__(self,h=224,w=224,c=3):
        super().__init__()
        self.proj_h=nn.Conv1d(
            h, h, kernel_size=1, stride=1, padding=0)
        self.proj_w=nn.Conv1d(
