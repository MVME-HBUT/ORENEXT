...
class DMLoss(nn.Module):
    ...
    def interpolation(self, poly, time=1):
        ...
        poly_interpolation = poly_ * time + poly_roll * (1. - time)
        poly_interpolation = poly_interpolation.permute(0, 1, 3, 2).reshape(poly_interpolation.size(3),
                                                                            ori_points_num , 1)
    ...
    def compute_distance(self, pred_poly, gt_poly):
        ...
        # pred_poly_expand = pred_poly_expand.expand(pred_poly_expand.size(0), gt_poly_expand.size(3),
        #                                            pred_poly_expand.size(2), pred_poly_expand.size(3))
        pred_poly_expand = pred_poly_expand.transpose(0,3)
        pred_poly_expand = pred_poly_expand.transpose(1,3)
        distance = torch.sum((pred_poly_expand - gt_poly_expand) ** 2, dim=3)
        return distance

    def compute_distance2(self, pred_poly, gt_poly):
    ...
    def get_pred_targets_item2(self, preds, offsets, key_points, masks):
        ...
        distances = self.compute_distance2(key_points, preds)
        ...
        # valid = valid[masks]
        key_points = key_points.unsqueeze(1)
        offsets_target = (key_points[masks] - preds) / self.offsets_stride

        # valid = valid.transpose(0,2)
        return offsets, offsets_target.detach()
